...
// Define for cc2650
#define OUTPUT
#define pinMode(pin, mode) ti_lib_ioc_pin_type_gpio_output(pin)
#define digitalWrite(pin, val) ti_lib_gpio_write_dio(pin, val)
#define delayMicroseconds(val) clock_delay_usec(val)
#define delay(val) delayMicroseconds(val * 1000)
...
MEMB(lcdDataStructs, struct lcdDataStruct, MAX_LCDS);
...
int lcdInit(const int rows, const int cols, const int bits, const int rs,
  ...
  // 4-bit mode?
  //	OK. This is a PIG and it's not at all obvious from the documentation I
  //had,
  //	so I guess some others have worked through either with better
  //documentation
  //	or more trial and error... Anyway here goes:
  //
  //	It seems that the controller needs to see the FUNC command at least 3
  //times
  //	consecutively - in 8-bit mode. If you're only using 8-bit mode, then it
  //appears
  //	that you can get away with one func-set, however I'd not rely on it...
  //
  //	So to set 4-bit mode, you need to send the commands one nibble at a
  //time,
  //	the same three times, but send the command to set it into 8-bit mode
  //those
  //	three times, then send a final 4th command to set it into 4-bit mode,
  //and only
  //	then can you flip the switch for the rest of the library to work in
  //4-bit
